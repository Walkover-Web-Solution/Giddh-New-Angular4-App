...
describe('This is Login Test', function() {
    ...
    it('Verify Trial Balance using Global Search', function () {
        cy.globalSearch('.nav-item.active > .nav-link > span', 'trial balance', 'Trial Balance')

    });

    it('Verify Invoice using Global Search', function () {
        cy.globalSearch('.navbar_header > .nav > .hamburger-menu > .inline','Invoice', 'Invoice ')

    });

    it('Verify Customer module using Global Search', function () {
        cy.globalSearch('.navbar_header > .nav > .hamburger-menu > .inline','Customer', 'Customer ')
    });

    it('Verify Vendor module using Global Search', function () {
        cy.globalSearch('.navbar_header > .nav > .hamburger-menu > .inline','Vendor', 'Vendor ')
    });

    it('Verify Settings module using Global Search', function () {
        cy.globalSearch('.navbar_header > .nav > .hamburger-menu > .inline','Settings', 'Settings > Taxes ')
    });

    it('Verify Walkover Account module using Global Search', function () {
        cy.globalSearch('.navbar_header > .nav > .hamburger-menu > .inline','Walkover', 'Walkover Technologies Private Limited A/c')
    });
           ...
           // cy.log(text, '\n')
            if (text === 'Cash'){
               // cy.log("In If statement")
                ...
                cy.get('.navbar_header > .nav > .hamburger-menu > .inline', {timeout: 50000}).should('be.visible')
                cy.wait(2000)
                cy.get('.navbar_header > .nav > .hamburger-menu > .inline', {timeout: 50000}).then((elementText) => {
                    cy.wait(5000).then(() =>{
                        const text = elementText.text();
                        cy.log(text)
                        expect(text).to.eq("Cash A/c")
                    })
