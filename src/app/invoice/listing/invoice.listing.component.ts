...
export class InvoiceListingComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public prepareModelForInvoiceReceiptApi(isUniversalDateSelected): InvoiceReceiptFilter {
    ...
    if (o.grandTotalMeasure === COMPARISON_FILTER[0].value) {
      model.totalMoreThan = true;
    } else if (o.grandTotalMeasure === COMPARISON_FILTER[1].value) {
      model.totalLessThan = true;
    } else if (o.grandTotalMeasure === COMPARISON_FILTER[2].value) {
      model.totalMoreThan = true;
      model.totalEqual = true;
    } else if (o.grandTotalMeasure === COMPARISON_FILTER[3].value) {
      model.totalLessThan = true;
      model.totalEqual = true;
    } else if (o.grandTotalMeasure === COMPARISON_FILTER[4].value) {
      model.totalEqual = true;
    }
  ...
  public dueDateChange(event: any) {
    if (event) {
      this.invoiceSearchRequest.dueDate = moment(event.picker.startDate._d).format(GIDDH_DATE_FORMAT);
      this.invoiceSearchRequest.dueDateAfter = true;
    }
  }
