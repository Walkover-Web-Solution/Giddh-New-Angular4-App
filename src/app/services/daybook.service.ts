...
export class DaybookService {
  ...
  public GetDaybook(request: DayBookRequestModel,fromdate:string,todate:string): Observable<BaseResponse<DayBookApiModel, DayBookRequestModel>> {
    this.user = this._generalService.user;
    this.companyUniqueName = this._generalService.companyUniqueName;
    return this._http.get(this.config.apiUrl + DAYBOOK_SEARCH_API.SEARCH
      .replace(':companyUniqueName', encodeURIComponent(this.companyUniqueName))
      .replace(':fromDate', encodeURIComponent(fromdate))
      .replace(':toDate', encodeURIComponent(todate)))
      .map((res) => {
        let data: BaseResponse<DayBookApiModel, DayBookRequestModel> = res.json();
        data.request = request;
        return data;
      })
      .catch((e) => this.errorHandler.HandleCatch<DayBookApiModel, DayBookRequestModel>(e, request));
  }
}