...
export class DaybookAdvanceSearchModelComponent implements OnInit, OnChanges, OnDestroy {
    ...
    public emitAdvanceSearchParams(): void {
        ...
        let fromDate = this.fromDate ?? this.selectedDateRange?.startDate ?? this.selectedDateRange.startDate.format(GIDDH_DATE_FORMAT);
        let toDate = this.toDate ?? this.selectedDateRange?.endDate ?? this.selectedDateRange.endDate.format(GIDDH_DATE_FORMAT);
        this.closeModelEvent.emit({
            action: 'search',
            exportAs: null,
            dataToSend,
