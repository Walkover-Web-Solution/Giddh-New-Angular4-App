...
import { ConfirmModalComponent } from 'apps/web-giddh/src/app/theme/confirm-modal/confirm-modal.component';
...
export class TriggersComponent implements OnInit, OnDestroy {
    ...
    @ViewChildren('variableComponent') public variableComponent: QueryList<any>;
    /** Holds image path */
    ...
    private getCommunicationPlatforms(): void {
        ...
        this.campaignIntegrationService.getCommunicationPlatforms().pipe(takeUntil(this.destroyed$)).subscribe(response => {
            if (response?.status === "success") {
                ...
                // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", response?.message);
                this.isCommunicationPlatformsLoading = false;
            }
        });
    }
        ...
        dialogRef?.afterClosed().subscribe(response => {
            if (response) {
                this.campaignIntegrationService.deleteCommunicationPlatform(platformUniqueName).pipe(takeUntil(this.destroyed$)).subscribe(response => {
                    if (response?.status === "success") {
                        // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("success", response?.body);
                        this.getCommunicationPlatforms();
                    } else {
                        // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", response?.message);
                    }
    ...
    */
    ...
    public validateTriggerForm(): boolean {
        ...
        if (!this.createTrigger.title) {
            ...
            // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", this.localeData?.communication?.invalid_title);
            return false;
        }
        if (!this.createTrigger.condition.action[0]) {
            this.mandatoryFields.condition = true;
            // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", this.localeData?.communication?.invalid_action);
            return false;
        }
        if (!this.createTrigger.condition.entity) {
            this.mandatoryFields.entity = true;
            // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", this.localeData?.communication?.invalid_entity);
            return false;
        }
        if (!this.createTrigger.condition.subConditions[0]?.action[0]) {
            this.mandatoryFields.subConditions = true;
            // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", this.localeData?.communication?.invalid_sub_entity);
            return false;
        }
        if (!this.createTrigger.campaignDetails.campaignName) {
            this.mandatoryFields.campaignSlug = true;
            // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", this.localeData?.communication?.invalid_slug);
            return false;
        }
        if (!this.createTrigger.campaignDetails.to?.length) {
            this.mandatoryFields.triggerToChiplist = true;
            // COMMENTED OUT - COMPILATION ERROR: this.toasty.showSnackBar("error", this.localeData?.communication?.invalid_to);
            return false;
        }
        return true;
    }
