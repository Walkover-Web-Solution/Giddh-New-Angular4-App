...
export class BranchComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    public ngOnInit() {
        ...
        // COMMENTED OUT - COMPILATION ERROR: this.universalDate$ = this.store.pipe(select(state => state.session.applicationDate), takeUntil(this.destroyed$));
        ...
        */
        ...
        // COMMENTED OUT - COMPILATION ERROR: this.store.pipe(select(s => s.session.createCompanyUserStoreRequestObj), takeUntil(this.destroyed$)).subscribe(res => {
        // if (res) {
        //     if (res.isBranch) {
