...
export class PaymentReceiptComponent implements OnInit, OnDestroy {
    ...
    private loadBankCashAccounts(customerCurrency: string): void {
        this.salesService.getAccountsWithCurrency('cash, bankaccounts, loanandoverdraft', `${customerCurrency}, ${this.activeCompany?.baseCurrency}`).pipe(takeUntil(this.destroyed$)).subscribe(data => {
