...
export class MobileSearchCompanyComponent implements OnInit, OnDestroy {
    ...
    public filterCompanyList(ev: string): void {
        let companies: CompanyResponse[] = cloneDeep(this.allCompanies).filter(company => ((company.name && company.name.toLowerCase().includes(ev.toLowerCase())) || (company.alias && company.alias.toLowerCase().includes(ev.toLowerCase()))));
