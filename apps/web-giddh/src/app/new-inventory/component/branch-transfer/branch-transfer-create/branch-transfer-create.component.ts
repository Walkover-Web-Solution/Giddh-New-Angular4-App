...
export class BranchTransferCreateComponent implements OnInit, OnDestroy {
  ...
  /** Hold  stock list data */
  public allStockList: IOption[] = [];
  public searchedStockName: string = '';
  /** Hold  stock list data */
  ...
  public handleScrollEnd(event: any): void {
    const filteredData = this.filteredStockList()
    const increasedSize = this.stockList.length + 30;
    this.stockList = filteredData.length <= increasedSize ? filteredData : filteredData.slice(0, increasedSize);
  }

  public onSearchQueryChanged(event: any): void {
    this.searchedStockName = event;
    this.stockList = this.filteredStockList().slice(0, 30);
  }
    ...
    this.store.pipe(select(data => data.inventory.stocksList), takeUntil(this.destroyed$)).subscribe((response) => {
      if (response && !isEmpty(response)) {
        ...
        response.results.forEach(key => {
          this.allStockList.push({ label: key.name, value: key?.uniqueName, additional: key })
        })
        this.stockList = this.allStockList.slice(0, 30);
      }
    });
  }

  public filteredStockList(): Array<IOption> {
    if (this.searchedStockName) {
      return this.allStockList.filter(response => response.label.toLowerCase().includes(this.searchedStockName.toLowerCase()));
    } else {
