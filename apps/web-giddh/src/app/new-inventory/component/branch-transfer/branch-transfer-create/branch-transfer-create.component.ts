        ...
        this.inventoryService.getNewBranchTransfer(this.editBranchTransferUniqueName).pipe(takeUntil(this.destroyed$)).subscribe((response) => {
            ...
            if (response?.status === "success") {
                ...
                response.body?.products.forEach(product => {
                    const productFormGroup = this.initProductFormGroup(); // Create a new form group for each product
                    ...
                    productsArray.push(productFormGroup); // Add the product form group to the 'products' array
                });

                response.body?.sources.forEach(source => {
                    const sourcesFormGroup = this.initSourceFormGroup(); // Create a new form group for each product
                ...
                response.body?.destinations.forEach(destination => {
                    const destinationsFormGroup = this.initDestinationFormGroup(); // Create a new form group for each product
                    ...
                    destinationsFormGroup.get('warehouse').patchValue({
                        ...
                        destinationsArray.push(destinationsFormGroup);
                });
