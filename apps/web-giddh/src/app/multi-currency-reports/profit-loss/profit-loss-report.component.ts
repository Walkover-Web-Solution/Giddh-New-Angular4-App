            ...
            if (response) {                
                ...
                if (cogs) {
                    ...
                    Object.keys(cogs).forEach((cogsKey, i) => {
                        if (i === 0) {
                            Object.keys(cogs[cogsKey])?.filter(f => ['openingInventory', 'closingInventory', 'purchasesStockAmount', 'manufacturingExpenses', 'debitNoteStockAmount'].includes(f)).forEach(f => {
