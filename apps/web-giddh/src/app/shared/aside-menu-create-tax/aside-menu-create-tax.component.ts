...
export class AsideMenuCreateTaxComponent implements OnInit, OnChanges {
    ...
    ngOnInit() {
        ...
        this.store.pipe(select(p => p.general.flattenAccounts), takeUntil(this.destroyed$)).subscribe(res => {
            let arr: IOption[] = [];
            if (res) {
            //  let accountObject =   res.filter(accountObj => accountObj.parentGroups.some(parentGroupObj => parentGroupObj.uniqueName === 'currentassets' || parentGroupObj.uniqueName === 'currentliabilities')); // Include currentliabilities and currentassets group
