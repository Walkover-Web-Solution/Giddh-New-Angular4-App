...
export class SalesService {
    ...
    */
    ...
    public getAccountsWithCurrency(groups: string, currency?: string): Observable<any> {
        ...
        let contextPath = `${this.config.apiUrl}${SALES_API_V2.GET_ACCOUNTS_OF_GROUP_WITH_CURRENCY.replace(':companyUniqueName', companyUniqueName)}`.concat(`?group=${groups}&count=0`);
        if (currency) {
