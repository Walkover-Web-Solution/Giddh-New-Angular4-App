    ...
    */
    ...
    public removeAmountIfAccountRemoved(transaction: FormGroup, index: number): void {
        if (!transaction.get('account')?.value && (transaction?.get('isDiscountApplied')?.value || transaction?.get('isTaxApplied')?.value)) {
            ...
            // this.selectedIdx = transactionsFormArray.length - 2;

            // if (transaction?.get('isDiscountApplied')?.value) {
            //     let amountControl: FormGroup;
            ...
            //         }
            //     });
            //     // amountControl.get('amount').patchValue(amountControl.get('amount').value + transaction.value.amount); // my old logic
            //     const newAmount  = amountControl.get('amount').value + transaction.value.amount;
            //     amountControl.get('amount').patchValue(newAmount);
            //     amountControl.get('actualAmount').patchValue(newAmount);
    ...
    public calculateDiscount(discountType?: string, discountValue?: number): number {
        ...
        if (!discountEntryControl?.get('amount')?.value) {
            if (amount) {
                discountAmount = (discountType === 'PERCENTAGE') ? this.getPercentAmount(amount, discountValue) : discountValue
    ...
    public addNewEntry(amount: any, transactionObj: any, entryIndex: number): void {
        this.calculateAmount(amount, transactionObj, entryIndex);
    }
    ...
    public calculateAmount(amount: any, transactionObj: any, indx: number): any {
        ...
        if (indx === lastIndx && transactionObj.get('selectedAccount.name').value) {
            ...
            this.journalVoucherForm.get('voucherType').setValue(this.currentVoucher);
    ...
    public saveEntry(): any {
        ...
        if (this.totalCreditAmount === this.totalDebitAmount) {
            if (this.validatePaymentAndReceipt(data)) {
                ...
                const discountValue = data.transactions.find(transaction => transaction.type === 'by' && transaction.isDiscountApplied)?.amount;
                ...
                if (data.transactions?.length) {
                    data.transactions[0].amount = data.transactions[0].amount + discountValue;
