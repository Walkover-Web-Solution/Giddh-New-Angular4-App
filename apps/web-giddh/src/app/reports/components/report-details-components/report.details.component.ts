...
constructor(
    ...
    public goToDetailedSales(item: ReportsModel) {
        ...
        if (from != null && to != null) {
            this.router.navigate(['pages', 'reports', 'sales-detailed-expand'], { queryParams: { from: from, to: to, branchUniqueName: this.currentBranch?.uniqueName, interval: item.interval, selectedMonth: item.selectedMonth, salesPersonUniqueName: item.salesPerson?.uniqueName, groupBy: this.currentGroupBy() } });
        }
    }
